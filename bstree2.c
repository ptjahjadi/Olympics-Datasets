...
C File for stage2.c by Patrick Tjahjadi */
...
node_t* new_node (value_t values) {
    ...
    newnode->left = newnode->right = newnode->duplicate = NULL;
    return newnode;
}
...
node_t* insert(node_t *root, value_t values) {
    ...
    /* Stage 2: Data entries with the same name are instead pushed to the
    linked list and consider them duplicates */
    else if (strcmp(root->values.name, values.name) == 0){
        root->duplicate = insert(root->duplicate, values);
    }
...
void search(node_t *root, char *searchname, int *num_comparisons, int *found, FILE* fout) {
    ...
    /* Stage 2: Traverse through the linked list of the node to check for
    duplicates, so no further key comparisons are required */
    else if (strcmp(root->values.name, searchname) == 0) {
        /* Increment num_comparisons only if information of an athlete is
        ...
        *found = 1;
        ...
        search(root->duplicate, searchname, num_comparisons, found, fout);
    }
